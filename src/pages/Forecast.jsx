...
function groupByWeek(batches, weeks = 12) {
  ...
  const buckets = Array.from({ length: weeks }, (_, i) => {
    ...
    return { label: formatDate(weekStart).slice(0, 6), weekStart, weekEnd, batches: [], totalKg: 0 };
  });
...
export default function Forecast() {
  ...
  const chartData = {
    ...
    datasets: [{
      label: 'Stock ready (kg)',
      data: weeks.map(w => Number(w.totalKg.toFixed(2))),
      ...
      tooltip: {
        callbacks: {
          afterLabel: (ctx) => {
            ...
            return bucket.batches.map(b => `  · ${b.products?.name}: ${formatKg(b.current_weight_kg)}`).join('\n');
          },
        },
      },
    },
    scales: {
      y: { title: { display: true, text: 'kg available' }, beginAtZero: true },
  ...
  for (const b of sorted) {
    ...
    if (!byProduct[name]) byProduct[name] = { name, totalKg: 0, batches: [] };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="card p-5">
        <h3 className="text-sm font-semibold text-stone-700 mb-4">Stock ready by week (kg)</h3>
        <div className="h-64">
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-stone-100">
              ...
              <th className="text-left text-xs font-semibold text-stone-400 uppercase tracking-wide px-5 py-2.5">Total stock</th>
          ...
          <tbody>
            {Object.values(byProduct).map(({ name, totalKg, batches: bList }) => {
              ...
              return (
                <tr key={name} className="border-b border-stone-100 last:border-0 hover:bg-stone-50">
                  ...
                  <td className="px-5 py-3 text-sm text-stone-700">{formatKg(totalKg)}</td>
      ...
      <div>
        ...
        <div className="card overflow-hidden">
          {sorted.length === 0 ? (
            ...
            sorted.map(b => <TimelineCard key={b.id} batch={b} />)
          )}
        </div>
      </div>
    </div>
